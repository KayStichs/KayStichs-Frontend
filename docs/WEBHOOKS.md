# KayStichs Webhooks Documentation

Webhooks allow your application to receive real-time notifications about events that happen in the KayStichs platform. Instead of polling our API for updates, we'll send HTTP requests to your endpoint when specific events occur.

## Overview

Webhooks are a powerful way to integrate with KayStichs and build responsive applications. For example, you could:
...
app.post('/webhooks/kaystichs', (req, res) => {
  const event = req.body;

  // Verify signature
  const signature = req.headers['x-kaystichs-signature'];
...
1. Log in to the KayStichs Partner Dashboard
...
{
  "url": "https://yourapp.com/webhooks/kaystichs",
...
```json
{
  ...
  "data": {
    ...
    "verifiableUrl": "https://verify.kaystichs.io/cred_789",
    "metadata": {
      "score": 100,
      "version": "1.0",
      "issuer": "KayStichs"
...
All webhook requests include a signature header that you must verify to ensure the request came from KayStichs.

**Header:** `X-KayStichs-Signature`
...
app.post('/webhooks/kaystichs', (req, res) => {
  ...
  const secret = process.env.KAYSTICHS_WEBHOOK_SECRET;
...
# Usage in Flask
@app.route('/webhooks/kaystichs', methods=['POST'])
def webhook():
    signature = request.headers.get('X-KayStichs-Signature')
    body = request.get_data(as_text=True)
    secret = os.environ.get('KAYSTICHS_WEBHOOK_SECRET')
...
For additional security, you can whitelist KayStichs's IP addresses:
...
```javascript
app.post('/webhooks/kaystichs', async (req, res) => {
...
async function sendWelcomeEmail(email, moduleTitle, score) {
  const msg = {
    ...
    html: `
      ...
      <a href="https://kaystichs.io/learn">Continue Learning →</a>
    `,
  };
...
- **Documentation**: [https://docs.kaystichs.io/webhooks](https://docs.kaystichs.io/webhooks)
- **Discord**: #webhooks channel
- **Email**: kaystichs@toneflix.net
- **Status Page**: [https://status.kaystichs.io](https://status.kaystichs.io)
